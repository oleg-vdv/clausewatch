...
> tsx src/cli.ts --profile support-agent --no-llm

# Tool-calling support agent
LLM agent that reads tickets and executes refunds through an internal API. The case none of these texts were drafted for.
Role: Deployer · Risk class: unknown · Jurisdictions in scope: EU · Agentic: takes actions, not only predictions
...
How long:
- Floor: at least 6 months — EU AI Act Art. 26(6) [binds the Deployer], in force from 2027-12-02 <https://artificialintelligenceact.eu/article/26/>
  Applies when: Deployer role; logs under the deployer’s control. Applies from 2 Dec 2027 for Annex III systems.
